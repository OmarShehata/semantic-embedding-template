...
const api = new OpenAI({
    apiKey: process.env.OPEN_API_KEY || 'lm-studio',
    baseURL: 'http://localhost:1234/v1',
});

export default class Embeddings {
    constructor({ id, dataPath = path.join(__dirname, '..', `.data/`) }) {
        this.id = id;
    ...
    async insertText(textArray) {
        ...
        const filteredText = textArray.filter(
            (text) => existingMap[text] == null,
        );
